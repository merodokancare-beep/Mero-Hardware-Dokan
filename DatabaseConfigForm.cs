...
namespace MeroHardwareDokan
{
    public class DatabaseConfigForm : Form
    {
        ...
        private void InitializeComponent()
        {
            ...
            comboAuth.Items.AddRange(new string[] { "Windows Authentication", "SQL Server Authentication" });
        ...
        private DatabaseHelper.DbConfig GetConfigFromUI()
        {
            ...
            if (int.TryParse(txtRetryCount.Text, out int retryCount))
                config.ConnectRetryCount = retryCount;

            if (int.TryParse(txtRetryInterval.Text, out int retryInterval))
        ...
        private void BtnTest_Click(object sender, EventArgs e)
        {
            ...
            // Run testing asynchronously to keep UI responsive
            string testConnString = DatabaseHelper.BuildConnectionString(GetConfigFromUI());
            
            // Wait, we need to test Master database to see if we can at least reach the server,
            // or directly test the initial catalog. Let's test the target database itself first.
