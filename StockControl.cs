...
namespace MeroHardwareDokan
{
    public class StockControl : UserControl
    {
        ...
        private void InitializeComponent()
        {
            ...
            txtSearch.PlaceholderText = "Search by Code or Name...";
            ...
            chkLowStockOnly.Location = new Point(340, 70);
            ...
            this.Controls.Add(chkLowStockOnly);

            // GridView
            gridStock = new DataGridView();
            gridStock.Size = new Size(910, 460);
            gridStock.Location = new Point(20, 115);
        ...
        private void LoadStock()
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(DatabaseHelper.ConnectionString))
                {
                    conn.Open();
                    string query = @"
                        SELECT Id, Code, Name, Category, Unit, Stock as [Current Stock], MinStockLevel as [Min Level],
                ...
                if (gridStock.Columns["Current Stock"] != null) gridStock.Columns["Current Stock"].DefaultCellStyle.Format = "0.###";
                if (gridStock.Columns["Min Level"] != null) gridStock.Columns["Min Level"].DefaultCellStyle.Format = "0.###";
                if (gridStock.Columns["Unit"] != null) gridStock.Columns["Unit"].HeaderText = "Unit";
            }
            catch (Exception ex)
            {
        ...
        private void BtnAdjust_Click(object sender, EventArgs e)
        {
            ...
            decimal currentStock = Convert.ToDecimal(selectedRow.Cells["Current Stock"].Value);
            string unit = selectedRow.Cells["Unit"].Value?.ToString() ?? "Pcs";

            using (AdjustStockDialog dlg = new AdjustStockDialog(id, code, name, currentStock, unit))
            {
        ...
        private class AdjustStockDialog : Form
        {
            ...
            public AdjustStockDialog(int id, string code, string name, decimal curStock, string unit)
            {
                this.productId = id;
                this.currentStock = curStock;
                this.unit = unit;
                InitializeComponent(code, name);
                this.ActiveControl = txtNewStock;
            }
            ...
            private void InitializeComponent(string code, string name)
            {
                ...
                lblCurrentStock.Text = $"Current Stock Level: {currentStock:0.###} {unit}";
                ...
                lblNew.Text = $"New Absolute Stock Count ({unit}) *";
                ...
                txtNewStock.Text = currentStock.ToString("0.###");
                ...
                txtRemarks.PlaceholderText = "E.g., Inventory Audit, Stock Damaged, etc.";
            ...
            {
                if (!decimal.TryParse(txtNewStock.Text.Trim(), out decimal newStock) || newStock < 0)
                {
                    MessageBox.Show("Please enter a valid non-negative numeric value for new stock.", "Validation Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
